...
def newmob():   #new method so that we wont have to repeat ourselves
    m = Mob()
    all_sprites.add(m)
    mobs.add(m)

def draw_shield_bar(surf, x, y, pct):   #this is the function that we are going to use to create the shield bar
    if pct < 0:
        pct = 0
    bar_length = 100
    bar_height = 10
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.shield = 100   #player starts off with a 100 shield
    ...
    def update(self):   #remember, this method is called one everytime the game updates(called in the update section below)
        ...
        if keystate[pygame.K_RIGHT]:    #if you want to use the d key, do K_d
            self.speedx = 5
        if keystate[pygame.K_SPACE]:    #calling the fire button here now
            self.shoot()
    ...
    def shoot(self):
        now = pygame.time.get_ticks()   #this will come in handy later in order to track time
...
class Explosion(pygame.sprite.Sprite):  #new explosion class
    def __init__(self, center, size):
        ...
        self.frame_rate = 50    #this adjusts how fast the explosion takes place
...
while running:
    ...
    hits = pygame.sprite.spritecollide(player, mobs, True, pygame.sprite.collide_circle)  #change it to true
    for hit in hits:    #if a list is empty, it is false. if hits has something, then it is true
        player.shield -= hit.radius * 2 #this will take out some of our health
        expl = Explosion(hit.rect.center, 'sm')
        all_sprites.add(expl)
        newmob()    #creating a new method that we wont dry
        if player.shield <= 0:  #if the player is dead, end the game
            running = False
    ...
    draw_shield_bar(screen, 5, 5, player.shield)    #this creates the health bar
